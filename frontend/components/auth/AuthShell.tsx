...
type AuthShellProps = { children: ReactNode; eyebrow: string; title: string; description: string; footer: ReactNode };
const enter = { hidden: { opacity: 0, y: 14 }, visible: { opacity: 1, y: 0 } };

export default function AuthShell({ children, eyebrow, title, description, footer }: AuthShellProps) {
  return <main className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-white">
    <div className="grid min-h-screen lg:grid-cols-[minmax(0,1.05fr)_minmax(480px,0.95fr)]">
      <aside className="relative hidden overflow-hidden bg-emerald-950 px-10 py-10 text-white lg:flex lg:flex-col xl:px-16">
        ...
        <Link href="/" className="relative z-10 flex w-fit items-center gap-3 rounded-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-emerald-950"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white/10 ring-1 ring-white/20"><Wallet className="h-5 w-5" /></span><span className="text-xl font-bold tracking-tight">Monetra</span></Link>
        <div className="relative z-10 my-auto max-w-md pt-20"><p className="mb-5 text-sm font-semibold uppercase tracking-[0.18em] text-emerald-300">Clarity for every decision</p><h2 className="text-4xl font-bold leading-tight tracking-tight xl:text-5xl">A calmer way to manage your money.</h2><p className="mt-6 max-w-sm text-base leading-7 text-emerald-50/75">Track the everyday, see the bigger picture, and make confident financial choices from one secure place.</p></div>
        <div className="relative z-10 grid max-w-md grid-cols-2 gap-3"><div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 backdrop-blur-sm"><BarChart3 className="mb-5 h-5 w-5 text-emerald-300" /><p className="text-sm font-semibold">Clear insights</p><p className="mt-1 text-xs leading-5 text-emerald-50/65">Understand your spending at a glance.</p></div><div className="rounded-2xl border border-white/10 bg-white/[0.07] p-4 backdrop-blur-sm"><ShieldCheck className="mb-5 h-5 w-5 text-emerald-300" /><p className="text-sm font-semibold">Private by design</p><p className="mt-1 text-xs leading-5 text-emerald-50/65">Your financial data stays yours.</p></div></div>
      </aside>
